...
func runVmTests(t *testing.T, tests []vmTestCase) {
	...
	for _, tt := range tests {
		...
		stackElemen := vm.LastPoppedStackElement()
		testExpectedObject(t, tt.expected, stackElemen)
	}
}
...
func testExpectedObject(
	...
	case string:
		...
		if err != nil {
			t.Errorf("testStringObjet failed %s", err)
		}
	case []int:
	...
	case map[object.HashKey]int64:
		...
		for expectedKey, expectedValue := range expected {
			...
			if err != nil {
				t.Errorf("testIntegerObject (hash) failed: %s", err)
			}
		}
	case *object.Null:
		if actual != Null {
			t.Errorf(
				"object is not null %T (%+v)",
				actual,
				actual,
			)
		}
...
func testBooleanObject(expected bool, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf(
			"object has wrong value, got %T (%+v)",
			result.Value,
			expected,
		)
	}
