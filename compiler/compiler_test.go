...
func TestCompilerScopes(t *testing.T) {
	...
	if compiler.scopeIndex != 1 {
		t.Errorf("scropeIndex wrong got %d want %d", compiler.scopeIndex, 1)
	}

	compiler.emit(code.OpSub)
	if len(compiler.scopes[compiler.scopeIndex].instructions) != 1 {
		t.Errorf(
			"instructions length wrong. got=%d",
	...
	if compiler.symbolTable.Outer != nil {
		t.Error("compiler modifies global symbol table incorrectly")
	}
...
func testInstructions(
	...
	if len(actual) != len(concatted) {
		return fmt.Errorf(
			"wrong instructions length was: %q got: %q",
			concatted,
			actual,
		)
	}
	...
	for i, ins := range concatted {
		if actual[i] != ins {
			return fmt.Errorf(
				"wrong instruction at %d want %q and got %q",
				i,
				concatted,
				actual,
			)
...
func testConstants(
	...
	for i, constant := range expected {
		...
		case string:
			...
			if err != nil {
				return fmt.Errorf(
					"constand %d - testStringObject failed: %s",
					i,
					err,
				)
			}
...
func testStringObject(expected string, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf(
			"object has wrong value, got %q want %q",
			result.Value,
			expected,
		)
	}
